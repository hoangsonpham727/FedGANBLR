...
from base_models.KDependenceBayesian import MLE_KDB, DiscBN_KDB, _compute_strides, _build_kdb_structure, _topo_order_from_parents
...
def _local_cpt_counts(data: np.ndarray, card: list[int], y_index: int, parents_full: dict[int, list[int]],
                      n_threads: Optional[int] = None) -> dict[int, np.ndarray]:
    """
    ...
    if len(feats) > 1 and n_threads > 1:
        with ThreadPoolExecutor(max_workers=n_threads) as executor:
            futures = {executor.submit(_counts_for_feature, v): v for v in feats}
            for fut in as_completed(futures):
                v, counts = fut.result()
                out[v] = counts
    else:
        for v in feats:
            v, counts = _counts_for_feature(v)
            out[v] = counts
...
class GANBLRFederatedClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        # Determine which weight computation method to use
        # Default to "thetas" (model parameters) if not specified
        
        use_theta_weights = config.get("use_theta_weights", True)
        
        # COMPUTE WEIGHTS USING EMPIRICAL COUNTS OR MODEL PARAMETERS
        if thetas_glob:
            ...
            if use_theta_weights:
                # COMPUTE WEIGHTS USING MODEL PARAMETERS (thetas)
                ...
                kl_s_weights = _mixture_kl_weights(
                    counts_loc, 
                    thetas_glob, 
                    alpha_mix=alpha_mix, 
                    beta=beta_pow, 
...
class KDBGANStrategy(fl.server.strategy.FedAvg):
    ...
    def __init__(
        ...
        # Initialize CSV file with headers if it doesn't exist (when enabled)
        if self.nll_csv_path and (not os.path.exists(self.nll_csv_path)):
            with open(self.nll_csv_path, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(["round", "client_id", "nll"])
        # Store model snapshots after each round for saving best/final models
        self.model_snapshots = {}  # {round: deepcopy of global_model}
    ...
    def aggregate_fit(self, rnd, results, failures):
        ...
        # Store model snapshot for this round (deep copy to avoid reference issues)
        import copy
        self.model_snapshots[int(rnd)] = {
            "py": np.copy(self.global_model["py"]),
            "thetas": {v: np.copy(t) for v, t in self.global_model["thetas"].items()},
            "s_y": np.copy(self.global_model.get("s_y", np.array([]))),
            "weights": np.copy(self.global_model.get("weights", np.array([]))) if isinstance(self.global_model.get("weights"), np.ndarray) else self.global_model.get("weights"),
            "client_kls": np.copy(self.global_model.get("client_kls", np.array([]))) if isinstance(self.global_model.get("client_kls"), (list, np.ndarray)) else self.global_model.get("client_kls"),
            "client_ns": np.asarray([cm.get("n", 0) for cm in client_models], dtype=np.int64),
            "nll_clients": len(nll_data),
        }
        
        # Log NLL values to CSV file (when enabled)
        if self.nll_csv_path and nll_data:
            try:
                # Write NLL data to CSV (append mode)
                ...
                # If CSV writing fails, continue without crashing
                warnings.warn(f"Failed to write NLL data to CSV: {e}")
        ...
        return fl.common.ndarrays_to_parameters([]), {
            ...
            "n_clients_with_nll": len(nll_data),
        }
