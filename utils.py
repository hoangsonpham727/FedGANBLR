...
def preprocess_covertype_binary_columns(X_df: pd.DataFrame) -> pd.DataFrame:
    """
    Preprocess Covertype dataset by combining binary columns into categorical indices.
    
    - Wilderness_Area1-4: Combined into single 'Wilderness_Area' categorical (0-3)
    - Soil_Type1-40: Combined into single 'Soil_Type' categorical (0-39)
    
    Args:
        X_df: Input DataFrame with binary columns
        
    Returns:
        DataFrame with binary columns replaced by categorical indices
    """
    X = X_df.copy()
    
    # Combine Wilderness_Area binary columns (Wilderness_Area1-4)
    wilderness_cols = [col for col in X.columns if col.startswith('Wilderness_Area') and col != 'Wilderness_Area']
    if wilderness_cols:
        # Sort to ensure consistent ordering (Wilderness_Area1, Wilderness_Area2, etc.)
        wilderness_cols = sorted(wilderness_cols, key=lambda x: int(x.replace('Wilderness_Area', '')))
        wilderness_values = X[wilderness_cols].values
        # Find which column is 1 for each row (argmax returns first max, which is what we want)
        wilderness_idx = np.argmax(wilderness_values, axis=1)
        # Handle case where all are 0 (shouldn't happen in Covertype, but safety check)
        all_zero_mask = wilderness_values.sum(axis=1) == 0
        if all_zero_mask.any():
            # If all zeros, set to -1 (will be treated as missing/other category)
            wilderness_idx[all_zero_mask] = -1
        X['Wilderness_Area'] = wilderness_idx
        X = X.drop(columns=wilderness_cols)
    
    # Combine Soil_Type binary columns (Soil_Type1-40)
    soil_cols = [col for col in X.columns if col.startswith('Soil_Type') and col != 'Soil_Type']
    if soil_cols:
        # Sort to ensure consistent ordering (Soil_Type1, Soil_Type2, etc.)
        soil_cols = sorted(soil_cols, key=lambda x: int(x.replace('Soil_Type', '')))
        soil_values = X[soil_cols].values
        # Find which column is 1 for each row
        soil_idx = np.argmax(soil_values, axis=1)
        # Handle case where all are 0
        all_zero_mask = soil_values.sum(axis=1) == 0
        if all_zero_mask.any():
            soil_idx[all_zero_mask] = -1
        X['Soil_Type'] = soil_idx
...
def _mdlp_discretize_1d(x, y):
    ...
    def _recurse(xs, ys):
        ...
        yl, yr, H, H_l, H_r = best_split if not None else 0,0,0,0,0
