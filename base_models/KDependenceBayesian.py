...
class MLE_KDB:
    ...
    def fit(self, data: np.ndarray, n_threads: Optional[int] = None):
        ...
        # decide threads for per-feature CPTs
        if n_threads is None:
            n_threads = os.cpu_count() or 8
        feats = [v for v in range(V) if v != self.y_index]

        def _fit_feature(v: int):
            """Compute CPT counts/theta for a single feature v."""
            ...
            return v, theta, strides, pa

        if len(feats) > 1 and n_threads > 1:
            # Parallelize over features
            with ThreadPoolExecutor(max_workers=n_threads) as executor:
                futures = {executor.submit(_fit_feature, v): v for v in feats}
                for fut in as_completed(futures):
                    v, theta, strides, pa = fut.result()
                    self.theta[v] = theta
            ...
            # Sequential fallback
            for v in feats:
                v, theta, strides, pa = _fit_feature(v)
                self.theta[v] = theta
                self.strides[v] = strides
                self.parent_lists[v] = pa
